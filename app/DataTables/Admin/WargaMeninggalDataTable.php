...
class WargaMeninggalDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('show', function($row){
                $warga = WargaMeninggal::where('warga_id', $row->id)->first();
                return view('pages.admin.warga.meninggal.show', ['warga' => $warga]);
            })
     ...
     *
     * @param \App\App\Models\Admin/WargaMeninggalDataTable $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(Warga $model)
    {
        return $model->with(['keluarga', 'agama', 'status_keluarga'])->select('warga.*')->where('status_warga_id', 3)->newQuery();
    }
    ...
    {
        return $this->builder()
                    ->setTableId('wargameninggaldatatable-table')
                    ...
                    ->parameters([
                        ...
                        $("#wargameninggaldatatable-table").on("click", ".info", function(){
    ...
    {
        return 'WargaMeninggal_' . date('YmdHis');
    }
}
