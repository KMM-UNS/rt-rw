...
<!-- begin panel -->
<form action="{{ isset($data) ? route('user.kas-rt.kas-iurankondisional.update', $data->id) : route('user.kas-rt.kas-iurankondisional.store') }}" id="form" name="form" method="POST" data-parsley-validate="true"  enctype="multipart/form-data">
    @csrf
    @if(isset($data))
    {{ method_field('PUT') }}
    @endif
    ...
    <div class="panel panel-inverse">
      ...
      {{-- <div class="panel-body">
        ...
        <div class="form-group">
            ...
            <x-form.Dropdown name="kas_iuran_kondisionals_bulan" :options="$nama_bulan" selected="{{{ old('kas_iuran_kondisionals_bulan') ?? ($data['bulan'] ?? null) }}}" required />
        </div>
          <div class="form-group">
