...
<!-- begin panel -->
<form action="{{ isset($data) ? route('user.kas-rt.kas-iuranwajib.update', $data->id) : route('user.kas-rt.kas-iuranwajib.store') }}" id="form" name="form" method="POST" data-parsley-validate="true"  enctype="multipart/form-data">
    @csrf
    @if(isset($data))
    {{ method_field('PUT') }}
    @endif
